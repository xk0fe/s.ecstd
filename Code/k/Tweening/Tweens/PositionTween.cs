...
public class PositionTween : TweenBase
{
	...
	public PositionTween(GameObject target, float duration, Vector3 from, Vector3 to, EasingType easing,
	...
	protected override async Task Play(bool forward)
	{
		if ( _duration <= 0 )
		{
			_target.WorldPosition = forward ? _to : _from;
			return;
		}
		...
		while (timeSince < _duration)
		{
			...
			_target.WorldPosition = Vector3.Lerp(start, end, easingFunc(progress));
			await GameTask.Yield();
		}

		_target.WorldPosition = end;
	}
}
